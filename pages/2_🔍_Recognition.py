...
st.set_page_config(page_title="Recognition Events", page_icon="🔍", layout="wide")
...
if not events:
    st.info("No recognition events found. Events appear when the recognition pipeline processes camera frames.")
else:
    for ev in events:
        with st.container():
            ...
            with cols[2]:
                ...
                st.markdown(f"""
                <span class="conf-badge {conf_class(v_conf)}">Vehicle: {v_conf:.0%}</span>
                <span class="conf-badge {conf_class(p_conf)}">Plate: {p_conf:.0%}</span>
                <span class="conf-badge {conf_class(o_conf)}">OCR: {o_conf:.0%}</span>
                """, unsafe_allow_html=True)

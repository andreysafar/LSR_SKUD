...
st.set_page_config(page_title="Cameras", page_icon="📷", layout="wide")
...
if not cameras:
    ...
    for cam in cameras:
        status_color = {"online": "#27AE60", "offline": "#E74C3C", "error": "#F39C12"}.get(cam["status"], "#95a5a6")
        ...
        with st.container():
            ...
            <div class="cam-card">
                ...
                <div class="cam-detail">Weights (Vehicle): {cam.get('weights_vehicle', 'default')}</div>
                <div class="cam-detail">Weights (Plate): {cam.get('weights_plate', 'default')}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    st.info(
        "Live camera previews require active RTSP streams. "
        "Snapshots from the recognition pipeline will appear in the Recognition Events page."
    )
    ...
    for i, cam in enumerate(cameras):
        with cols[i % 3]:
            ...
            if os.path.exists(snapshots_dir):
                snapshots = sorted(
                    [f for f in os.listdir(snapshots_dir) if f.startswith(snapshot_pattern) and not f.endswith("_plate.jpg")],
                    reverse=True
                )
                if snapshots:
                    latest = os.path.join(snapshots_dir, snapshots[0])
                    st.image(latest, caption=f"Latest snapshot", use_container_width=True)
                else:
                    st.markdown(
